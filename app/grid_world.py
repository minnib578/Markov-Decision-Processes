...
class App(tk.Frame):
    ...
    def __init__(self, alg_fn_map, worlds, master=None):
        ...
        self.master.title("Markov Decision Process -- CPSC 4420/6420 Clemson University")
        ...
        self.text_cost = tk.Label(self.master, text=" ")
        ...
        self.text_cost.grid(row=4, column=1,
            sticky=tk.E, padx=10
        )
        ...
        self.cell_size = 0
        self.world_canvas.bind("<Configure>", lambda _: self.generate_world(next(v for k, v in self.worlds.items()), noise=0.2, cost=0.01))
    ...
    def solve(self):
        ...
        # cost_var = tk.StringVar(self.solve_window)
        # cost_var.set("0.01")
        # field_cost = tk.Entry(self.solve_window, textvariable=cost_var)
        ...
        def solve():
            ...
            #     failed = True
            # if failed:
            #     tk.messagebox.showinfo("", "Cost must be a valid number.")
            #     return
            ...
            self.generate_world(world, noise, cost=0)
            ...
            # self.text_cost.config(text="Action Cost: {}".format(cost))
        ...
        # tk.Label(self.solve_window,
        #     text="Action Cost", justify=tk.RIGHT).grid(
        #     row=4, column=0, padx=10, sticky=tk.E
        # )
        # field_cost.grid(
        #     row=4, column=1, padx=10, sticky=tk.W
        # )
        tk.Label(self.solve_window,
    ...
    def generate_world(self, generate_fn, noise, cost):
        ...
        self.env = self.Env(self, reward_map, starts=start, goals=goal, obstacles=obstacles, noise=noise)
        self.logger.clear()
        self.draw_world()
